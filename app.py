...
def parse_content(data: dict) -> dict[str, str]:
    ...
    for node in data["order"]:
        ...
        for paragraph in data["obj"].get(node, []):
            parsed_content[node] += paragraph
    return parsed_content


def build_html_page(data: dict, parsed_content: dict[str, str]) -> str:
    page = ""
    for idx, node in enumerate(data["order"]):
        tag = data["tags"][idx].lower()
        segment = f"<{tag}>{node.replace('_', ' ')}</{tag}>\n{parsed_content[node]}"
        page += segment
...
def main() -> None:
    ...
    st.write("Scrape a Wikipedia page and analyze it step by step.")
    ...
    user_input_tab, scrape_tab, analyze_tab = st.tabs(["User Input", "Scrape Page Content", "Analyze content"])
    ...
    with scrape_tab:
        ...
        if scrape_clicked:
            ...
            elif re.search(r"wikipedia\.org/wiki/.+", url.strip()) is None:
                st.error("Please enter a valid Wikipedia URL (must start with https://www.wikipedia.org/wiki/).")
            else:
                try:
    ...
    with analyze_tab:
        ...
        top_n = st.slider("Top N words", min_value=5, max_value=50, value=20)
        ...
        if analyze_clicked:
            try:
                with st.spinner("Analyzing content..."):
                    ...
                    top_words = [(w, c) for w, c in filtered_counts.most_common(top_n) if len(w) > 3]
        ...
        # st.header("Unify all Content")
        # st.caption(f"Unified text length: {analysis['unified_length']:,} characters")

        # st.header("Clean and Normalize Text")
        # st.caption(f"Cleaned text length: {analysis['cleaned_length']:,} characters")

        # st.header("Tokenize and Count Words")
        # st.caption(f"Unique words before stopword filtering: {analysis['unique_before']:,}")

        # st.header("Remove Stop Words")
        # st.caption(f"Unique words after stopword filtering: {analysis['unique_after']:,}")

        st.header("Identify Top N Words")
        if not top_words:
            st.warning("No words found after filtering. Try a different page or lower constraints.")
            return

        results_container = st.container(height=1000)
        with results_container:
            ...
            top_keywords = ", ".join([word for word, _ in top_words[:6]])
